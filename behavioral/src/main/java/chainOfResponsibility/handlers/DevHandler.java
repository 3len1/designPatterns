...
public class DevHandler extends Handler {
    ...
    public void handleRequest(Request request) {
        if (request.getType() == RequestType.DEVELOP)
            LOGGER.info("Request [" + request.getTittle() + "] successfully handled by DevHandler\n");
        else {
            LOGGER.warn("DevHandler can't handle request [" + request.getTittle() +
