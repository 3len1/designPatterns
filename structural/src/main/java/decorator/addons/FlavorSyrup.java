 ...
 */
public class FlavorSyrup extends Decorator {

    private static String DESCRIPTION = ", syrup";
    private static BigDecimal ADD_ON_PRICE = new BigDecimal(0.20);
    @Getter
    @Setter
    private Flavor flavor;

    public FlavorSyrup(Beverage beverage) {
        if (beverage == null)
    ...
    public FlavorSyrup(Beverage beverage, Flavor flavor) {
        if (beverage == null)
            throw new NotInitialized(BeverageType.class, "Decorator can't be initialize to nullable beverage");
        this.beverage = beverage;
        this.description = DESCRIPTION;
